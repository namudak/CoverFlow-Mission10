...
//		}
		...
		transformImageBitmap(childCenter,(ImageView) child, t, rotationAngle);
	...
	private void transformImageBitmap(int center, ImageView child, Transformation t, int rotationAngle) {
		camera.save();

		Matrix imageMatrix = t.getMatrix();

		final int imageHeight = child.getLayoutParams().height;
		final int imageWidth = (center== centerPoint)?
				child.getLayoutParams().width: (int)(child.getLayoutParams().width*0.15);
		final int rotation = (center== centerPoint) ?0 : Math.abs(rotationAngle);
...
//			imageMatrix.postScale(0.15f, 1.0f);
//			child.setImageMatrix(imageMatrix);
//		} else {
